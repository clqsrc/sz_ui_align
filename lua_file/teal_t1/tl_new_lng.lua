...
do
   ...
   function tl.lex(input, filename)  --//clq 即所谓的 "词法分析器" Lexical Analyzer Generato //如果只是改它的兼容性的话，只要改这个函数就可以了
      local tokens = {}
      ...
      local len = #input                         --//clq 这里是正式分析的开始，前面其实是子函数
      ...
      while i <= len do
         ...
         local c = input:sub(i, i)      --//clq 每次读取一个字节。 fwd 似乎表示是否向下读取的意思
         ...
         elseif state == "string double" then
            ...
            elseif c == "\"" then
               end_token_here("string")            --//clq 意思是说找到了字符串的结束符。并且这是双引号的字符串
               state = "any"
            end
         elseif state == "comment short" then      --//clq 短注释，即行内注释
            if c == "\n" then
               state = "any"
            end
